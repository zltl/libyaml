...
static inline int PUT(yaml_emitter_t* emitter, char value) {
    if (!FLUSH(emitter)) {
        return 0;
    }
    *(emitter->buffer.pointer++) = (yaml_char_t)(value), emitter->column++;
    return 1;
}
...
static inline int PUT_BREAK(yaml_emitter_t* emitter) {
    if (!FLUSH(emitter)) {
        return 0;
    }
...
static int WRITE(yaml_emitter_t* emitter, yaml_string_t string) {
    if (!FLUSH(emitter)) {
        return 0;
    }
...
static int WRITE_BREAK(yaml_emitter_t* emitter, yaml_string_t string) {
    if (!FLUSH(emitter)) {
        return 0;
    }
    if (CHECK(string, '\n')) {
