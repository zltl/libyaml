...
int main(int argc, char* argv[]) {
    ...
    if (!yaml_emitter_open(&emitter)) goto emitter_error;
        ...
        switch (input_event.type) {
            case YAML_STREAM_START_EVENT:
                ...
                if (input_event.data.stream_start.encoding) {
                    ...
                    if (!yaml_document_append_mapping_pair(
                            &output_document, properties, key, value))
                        goto document_error;
                }
            ...
            case YAML_DOCUMENT_START_EVENT:
                ...
                if (input_event.data.document_start.version_directive) {
                    ...
                    if (!yaml_document_append_mapping_pair(
                            &output_document, properties, key, map))
                        goto document_error;
                ...
                if (input_event.data.document_start.tag_directives.start !=
                    ...
                    if (!yaml_document_append_mapping_pair(
                            &output_document, properties, key, seq))
                        goto document_error;
                    ...
                    for (tag = input_event.data.document_start.tag_directives
                        ...
                        map = yaml_document_add_mapping(
                            &output_document, NULL, YAML_FLOW_MAPPING_STYLE);
                        if (!map) goto document_error;
                        if (!yaml_document_append_sequence_item(
                                &output_document, seq, map))
                            goto document_error;

                        /* Add 'handle': <handle>. */

                        key = yaml_document_add_scalar(
                            &output_document, NULL, (yaml_char_t*)"handle", -1,
                            YAML_PLAIN_SCALAR_STYLE);
                        if (!key) goto document_error;
                        value = yaml_document_add_scalar(
                        ...
                        key = yaml_document_add_scalar(
                            &output_document, NULL, (yaml_char_t*)"prefix", -1,
                            YAML_PLAIN_SCALAR_STYLE);
                        if (!key) goto document_error;
                        value = yaml_document_add_scalar(
            ...
            case YAML_ALIAS_EVENT:
                ...
                if (!key) goto document_error;
                value = yaml_document_add_scalar(
                    &output_document, NULL, input_event.data.alias.anchor, -1,
                    YAML_DOUBLE_QUOTED_SCALAR_STYLE);
                if (!value) goto document_error;
                if (!yaml_document_append_mapping_pair(&output_document,
                                                       properties, key, value))
            ...
            case YAML_SCALAR_EVENT:
                ...
                if (input_event.data.scalar.anchor) {
                    ...
                    if (!key) goto document_error;
                    value = yaml_document_add_scalar(
                        &output_document, NULL, input_event.data.scalar.anchor,
    ...
    if (!yaml_emitter_dump(&emitter, &output_document)) goto emitter_error;
    if (!yaml_emitter_close(&emitter)) goto emitter_error;
